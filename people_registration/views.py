...
def excluir_pessoa(request, pessoa_id):
    try:
        ...
        if request.method == 'DELETE':
            ...
            return JsonResponse({"sucesso": False, "mensagem": f"Método {request.method} não é permitido."}, status=404)
        
    except Exception as e:
        return JsonResponse({"sucesso": False, "mensagem": f"Erro{e}."}, status=404)
